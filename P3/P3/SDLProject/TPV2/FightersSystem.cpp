...
void FightersSystem::update() {
	...
	if(mngr_->getClientId() == 0) updateFighter(fighter0_);
	else if (mngr_->getClientId() == 1) updateFighter(fighter1_);
...
void FightersSystem::recieve(const msg::Message& msg)
{
	...
	case msg::_FIGHTER_INFO: {
		...
		msg::FighterInfoMessage info = static_cast<const msg::FighterInfoMessage&>(msg);

		Transform* tr_ = nullptr;
		if (msg.senderClientId == 0) tr_ = fighter0_->getComponent<Transform>(ecs::Transform);
		else tr_ = fighter1_->getComponent<Transform>(ecs::Transform);

		tr_->position_.set(Vector2D(info.x, info.y));
		tr_->rotation_ = info.rotation;
...
void FightersSystem::updateFighter(Entity *e) {
	...
	if (ih->keyDownEvent()) {
		if (ih->isKeyDown(keys->left)) { // turn left
			...
			mngr_->send<msg::BulletShootMessage>(p.getX(), p.getY(), d.getX(), d.getY(), 2, 5);
		}
	...
	mngr_->send<msg::FighterInfoMessage>(tr->position_.getX(), tr->position_.getY(), tr->rotation_);
}
